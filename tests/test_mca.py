...
class TestMca(TestCase):
	...
	def test_abdi_williams(self):
		...
		assert_allclose(mca_df.dr, [0.0630, 0.0474, 0.0116, 0.0084, 0.0543, 0.0408],
						atol=1e-4)

		# abdi = numpy.array([216, 139, 26]) #
		abdi = DataFrame([216, 139, 26]).T
		assert_allclose(mca_df.fs_r_sup(abdi, 2), [[-0.0908, 0.5852]], atol=1e-4)

		supp = read_table('data/french_writers_supp.csv', 
							skiprows=0, index_col=0, sep=',')
